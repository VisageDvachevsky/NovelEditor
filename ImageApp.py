...
class ImageApp:
    def __init__(self, master):
        ...
        self.browse_button = ttk.Button(self.master, text="Выбрать изображение", command=self.browse_image)
        self.browse_button.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky="we")

        self.add_button = ttk.Button(self.master, text="Добавить изображение", command=self.add_image)
        self.add_button.grid(row=2, column=0, columnspan=2, padx=10, pady=5, sticky="we")

        self.delete_button = ttk.Button(self.master, text="Удалить изображение", command=self.delete_image)
        self.delete_button.grid(row=3, column=0, columnspan=2, padx=10, pady=5, sticky="we")

        self.backup_button = ttk.Button(self.master, text="Создать резервную копию", command=self.backup_database)
        self.backup_button.grid(row=4, column=0, columnspan=2, padx=10, pady=5, sticky="we")
